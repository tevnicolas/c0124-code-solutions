...
app.post('/api/actors', async (req, res, next) => {
  try {
    ...
    if (!firstName || !lastName) {
      throw new ClientError(400, 'Must include first name and lastname.');
    }
    const sql = `
...
app.put('/api/actors/:actorId', async (req, res, next) => {
  try {
    const { actorId } = req.params;
    const { firstName, lastName } = req.body;
    if (!firstName || !lastName || !Number.isInteger(+actorId)) {
      throw new ClientError(
        400,
        'Must include first name and lastname and actorId.'
      );
    }
    const sql = `
      update "actors"
...
app.delete('/api/actors/:actorId', async (req, res, next) => {
  try {
    const { actorId } = req.params;
    if (!Number.isInteger(+actorId)) {
      throw new ClientError(400, 'Must include actorId.');
    }
    const sql = `
      delete
        from "actors"
